...
export async function validateArtifactPaths(paths: readonly string[], cwd: string): Promise<string[]> {
  ...
  for (const path of unique) {
    ...
    if (info === undefined) throw new Error(`artifact does not exist: ${path}`)
...
export const ARTIFACT_REGISTRATION_PROMPT =
  'When a shell command, Python script, notebook, or another tool without produced-file metadata creates final user-facing files, '
  + 'you must verify the files exist and then call artifact_register once with every final output file, including PDF files, before your final response. '
  + 'This registration makes each output clickable and available in the Files sidebar. '
  + 'Do not register files already reported by a file-writing tool. '
  + 'In the final response, reference the registered outputs using their exact paths.'

/** Register the prompt section paired with artifact_register. */
export function registerArtifactPrompt(ctx: Context): () => void {
...
export function registerArtifactTool(ctx: Context): () => void {
  return ctx.tools.register(defineTool({
    name: 'artifact_register',
    description:
      'Register files that were successfully created by a shell command, script, notebook, or other tool whose own result does not expose produced-file locations. '
      + 'Call this immediately after verifying those files exist so they become clickable deliverables and appear in the Files sidebar. '
    ...
    output: {
      ...
      render: textRender((value: { paths: string[] }) => `Registered ${String(value.paths.length)} generated file(s).`),
    },
