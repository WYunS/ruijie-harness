...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  // Receive model-issued browser_search/browser_open actions and land them
  // in the same conversation's visible sidebar. Commands are session scoped
  // by both the WebSocket query and the validated payload.
  useEffect(() => {
    if (current === undefined || ctx.betterSidebar === undefined) return
    let closed = false
    ...
    const connect = (): void => {
      ...
      socket.onmessage = (event) => {
        ...
        if (command === null || command.sessionId !== current || ctx.betterSidebar === undefined) return
