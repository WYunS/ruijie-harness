...
export function selectProducedFiles(owner: unknown): readonly string[] | null {
  ...
  const current = record.turn?.data?.get?.('deliverables') as { produced?: unknown } | undefined
  let paths: readonly string[]
  if (Array.isArray(current?.produced)) {
    const seen = new Set<string>()
    paths = current.produced.flatMap((item) => {
      if (item === null || typeof item !== 'object') return []
      const produced = item as { seq?: unknown; path?: unknown }
    ...
    // Compatibility with rc.7 replays. rc.8 owners carry the canonical
    // turn-scoped value above and no longer expose a flat nodes array.
    paths = producedForClosing(record.nodes, seq)
  } else {
    return null
  }
