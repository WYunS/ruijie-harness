...
export function normalizeBrowserUrl(input: string, selfOrigin: string): BrowserNavigateResult {
  ...
  // Match a normal browser omnibox: obvious addresses navigate directly;
  // ordinary words become a Baidu query. The previous implementation turned
  // every phrase into https://<phrase>, producing a slow blank page.
  const looksLikeAddress = /^(?:https?:\/\/)/iu.test(trimmed)
    || /^\[[0-9a-f:]+\](?::\d+)?(?:\/.*)?$/iu.test(trimmed)
    || /^(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?(?:\/.*)?$/u.test(trimmed)
    || /^(?:localhost|[^\s./]+(?:\.[^\s./]+)+)(?::\d+)?(?:\/.*)?$/iu.test(trimmed)
  if (!looksLikeAddress && !/^[a-zA-Z][a-zA-Z0-9+.-]*:/u.test(trimmed)) {
    return { kind: 'ok', url: `https://cn.bing.com/search?q=${encodeURIComponent(trimmed)}` }
  }
 ...
 */
export function fallbackSearchUrlForBlockedSite(input: string): string | undefined {
  let url: URL
  try { url = new URL(input) } catch { return undefined }
  const hostname = url.hostname.toLowerCase()
  if (hostname !== 'baidu.com' && !hostname.endsWith('.baidu.com')) return undefined
  ...
  if (hostname === 'baike.baidu.com') {
    ...
    if (topic !== undefined) {
      try { query = decodeURIComponent(topic).replaceAll('_', ' ') } catch { query = topic.replaceAll('_', ' ') }
    }
  }
