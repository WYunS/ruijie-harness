...
export function wrapOpenPath(workspaces: OpenPathService, deps: OpenPathInterceptDeps): () => void {
  ...
  workspaces.openPath = (path: string): Promise<void> => {
    // ui-deliverables uses '.' for its explicit "show in folder" action.
    // ui-conversation resolves that relative path against the session cwd
    // before this service boundary, so compare the absolute cwd as well.
    // Directory intents belong to the Host/OS, not the sidebar editor.
    const comparable = (value: string): string => {
      const normalized = value.replace(/[\\/]+/gu, '/').replace(/\/$/u, '')
      return /^[a-z]:\//iu.test(normalized) ? normalized.toLowerCase() : normalized
    }
    const cwd = deps.currentSessionCwd()
    const directoryIntent = /(?:^|[\\/])\.\.?[\\/]?$/u.test(path)
      || /[\\/]$/u.test(path)
      || (cwd !== undefined && comparable(path) === comparable(cwd))
