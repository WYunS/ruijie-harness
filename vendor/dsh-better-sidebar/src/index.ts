...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  // Shell/script output has no mutation-tool presentation metadata of its
  // own. This bridge lets the agent publish verified files as deliverables.
  ...
  // Model browser tools push one navigation command to the matching
  // conversation UI. This is deliberately a separate channel from agent
  // terminals: browser commands are transient actions, not persistent rows.
  ...
  ctx.effect(() => ctx.webServer.registerUpgrade({
    ...
    handler: (req, socket, head) => {
      ...
      browserCommandsWss.handleUpgrade(req as unknown as IncomingMessage, socket as unknown as Duplex, head as Buffer, (ws) => {
        void attachBrowserCommands(browserCommandBroker, ws, req)
      })
    },
...
async function attachBrowserCommands(
