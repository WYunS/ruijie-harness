...
//#region ../../vendor/dsh-better-sidebar/src/prefs-shared.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/config.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/wire.ts
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/fs-tree.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/fs-search.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/browser-probe.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/trust-fence.ts
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/bundle-route.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/git.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/pty-deps.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/pty-manager.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/agent-pty.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/tools.ts
/**
* Model-facing sidebar tools: eight agent-owned terminal controls (tmux
* semantics: spawn-and-detach, send-keys, read, wait-for, resize, signal,
* close, list), plus an artifact-registration bridge for files created by
* shell commands and scripts. Each tool binds to the calling agent's session through
...
/** Resolve and validate shell-created artifacts before publishing them. */
async function validateArtifactPaths(paths, cwd) {
	if (paths.length === 0) throw new Error("artifact_register requires at least one path");
	if (paths.length > ARTIFACT_LIMIT) throw new Error(`artifact_register accepts at most ${String(ARTIFACT_LIMIT)} paths`);
	const unique = [...new Set(paths.map((path) => isAbsolute(path) ? resolve(path) : resolve(cwd, path)))];
	for (const path of unique) {
		if (!isWithin(cwd, path)) throw new Error(`artifact is outside the session working directory: ${path}`);
		const info = await stat(path).catch(() => void 0);
		if (info === void 0) throw new Error(`artifact does not exist: ${path}`);
		if (!info.isFile()) throw new Error(`artifact is not a file: ${path}`);
	}
	return unique;
}
/** Stable guidance that closes the metadata gap left by shell and script tools. */
const ARTIFACT_REGISTRATION_PROMPT = "When a shell command, Python script, notebook, or another tool without produced-file metadata creates final user-facing files, you must verify the files exist and then call artifact_register once with every final output file, including PDF files, before your final response. This registration makes each output clickable and available in the Files sidebar. Do not register files already reported by a file-writing tool. In the final response, reference the registered outputs using their exact paths.";
/** Register the prompt section paired with artifact_register. */
function registerArtifactPrompt(ctx) {
...
/** Register the generic bridge used after a shell or script creates files. */
function registerArtifactTool(ctx) {
	return ctx.tools.register(defineTool({
		name: "artifact_register",
		description: "Register files that were successfully created by a shell command, script, notebook, or other tool whose own result does not expose produced-file locations. Call this immediately after verifying those files exist so they become clickable deliverables and appear in the Files sidebar. Do not call it for files already reported by a file-writing tool.",
		parameters: { paths: {
			type: "array",
			required: true,
			items: { type: "string" },
			description: "One or more existing file paths, absolute or relative to the current session working directory."
		} },
		output: {
			schema: {
				type: "object",
			...
			render: textRender((value) => `Registered ${String(value.paths.length)} generated file(s).`)
		},
		execute: async (args, exec) => {
			exec.signal.throwIfAborted();
			const cwd = requireAgent(exec.agent).session.header.cwd ?? process.cwd();
			return { paths: await validateArtifactPaths(args.paths, cwd) };
		},
		presentCall: (args) => ({
			card: "generic",
			kind: "edit",
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/browser-tools.ts
...
*/
var BrowserCommandBroker = class {
	nextId = 1;
...
function sessionIdOf(exec) {
	...
	if (sessionId === void 0 || sessionId === "") throw new Error("sidebar browser tools require an initiating agent session");
	return sessionId;
}
...
function validateHttpUrl(raw) {
	...
	if (url.protocol !== "http:" && url.protocol !== "https:") throw new Error("only http:// and https:// addresses can be opened");
	return url;
}
/** Register browser_search and browser_open for every agent session. */
function registerBrowserTools(ctx, broker) {
	const disposers = [];
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/jobs-routes.ts
/**
...
//#endregion
//#region ../../vendor/dsh-better-sidebar/src/index.ts
/**
...
function apply(ctx, config) {
	...
	const browserCommandBroker = new BrowserCommandBroker();
	const browserToolsDisposer = registerBrowserTools(ctx, browserCommandBroker);
	const artifactToolDisposer = registerArtifactTool(ctx);
	ctx.inject(["systemPrompt"], (promptCtx) => {
		registerArtifactPrompt(promptCtx);
	});
	let settingsFace;
	...
	const browserCommandsWss = new WebSocketServer({ noServer: true });
	ctx.effect(() => ctx.webServer.registerUpgrade({
		path: "/sidebar/ws/browser-commands",
		handler: (req, socket, head) => {
			if (!fence(req)) {
				socket.destroy();
				return;
			}
			browserCommandsWss.handleUpgrade(req, socket, head, (ws) => {
				attachBrowserCommands(browserCommandBroker, ws, req);
			});
		}
	}), "dsh-better-sidebar: browser command WebSocket");
	ctx.effect(() => () => {
		browserToolsDisposer();
		artifactToolDisposer();
		toolsDisposers?.();
		ptyManager?.disposeAll();
		agentPtyRegistry?.disposeAll();
		wss.close();
		agentListWss.close();
		browserCommandBroker.clear();
		browserCommandsWss.close();
	}, "dsh-better-sidebar: teardown");
}
/** Attach one conversation UI to model-issued browser commands. */
async function attachBrowserCommands(broker, ws, req) {
	try {
