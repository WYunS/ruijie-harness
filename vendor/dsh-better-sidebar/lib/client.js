window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/service.ts
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/chunk-loader.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/locales.ts
		/**
		...
		*/
		...
		const zh = {
			...
			viewerHtml: "HTML",
			browser: "浏览器",
			browserPlaceholder: "搜索或输入网址",
			browserGo: "前往",
			browserBack: "后退",
			browserForward: "前进",
			browserStart: "搜索或输入网址开始浏览",
			...
			settingsBrowserHttpsDesc: "点击聊天或界面中的 HTTPS 外链时默认在侧边栏打开；若网站禁止嵌入，可从提示中改用系统浏览器",
		...
		const en = {
			...
			producedShowFolder: "Show in folder",
			...
			viewerHtml: "HTML",
			browser: "Browser",
			browserPlaceholder: "Search or enter an address",
			browserGo: "Go",
			browserBack: "Back",
			browserForward: "Forward",
			browserStart: "Search or enter an address to browse",
			...
			settingsBrowserHttpsDesc: "HTTPS links in the chat or GUI open in the sidebar by default; sites that refuse embedding can be opened in the system browser from the notice",
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/paths.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/produced-files.ts
		/**
		...
		*/
		function selectProducedFiles(owner) {
			...
			const current = record.turn?.data?.get?.("deliverables");
			let paths;
			if (Array.isArray(current?.produced)) {
				const seen = /* @__PURE__ */ new Set();
				paths = current.produced.flatMap((item) => {
					if (item === null || typeof item !== "object") return [];
					const produced = item;
					if (typeof produced.seq !== "number" || produced.seq > seq) return [];
					if (typeof produced.path !== "string" || seen.has(produced.path)) return [];
					seen.add(produced.path);
					return [produced.path];
				});
			} else if (Array.isArray(record.nodes)) paths = producedForClosing(record.nodes, seq);
			else return null;
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/openpath-intercept.ts
		/**
		...
		*/
		function wrapOpenPath(workspaces, deps) {
			...
			workspaces.openPath = (path) => {
				...
				const cwd = deps.currentSessionCwd();
		...
		//#endregion
		//#region \0dsh-css:D:\ChatGPT\RuijieDSH\vendor\dsh-better-sidebar\src\client\sidebar.module.css.mjs
		const css$3 = "[data-dsh-panel-host]{z-index:40;pointer-events:none;position:fixed;inset:0}[data-dsh-panel-host][data-dsh-panel-host-degraded]{position:absolute;top:0;left:0}.vCHcua_toggleCluster{top:calc(3px + env(safe-area-inset-top));z-index:45;pointer-events:auto;flex-direction:row;gap:4px;display:flex;position:absolute;right:10px}.vCHcua_panel:not(.vCHcua_panelHidden) .vCHcua_tabBar{padding-right:72px}.vCHcua_toggleButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;display:flex}.vCHcua_toggleButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_toggleButton:disabled{opacity:.4;cursor:default}.vCHcua_panel{z-index:40;pointer-events:auto;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;top:0;bottom:0;right:0}.vCHcua_panelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translate(102%)}.vCHcua_panel[data-dragging]{transition:none}.vCHcua_panelResize{cursor:col-resize;z-index:2;touch-action:none;width:8px;position:absolute;top:0;bottom:0;left:-4px}.vCHcua_panelResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.vCHcua_panelBody{flex:1;min-width:0;min-height:0;display:flex}.vCHcua_bottomPanel{z-index:40;background:var(--dsw-alias-bg-layer-1);border-top:1px solid var(--dsw-alias-border-l2);pointer-events:auto;padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;bottom:0}.vCHcua_bottomPanelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translateY(102%)}.vCHcua_bottomPanel[data-dragging]{transition:none}.vCHcua_panel,.vCHcua_bottomPanel{contain:layout style}body[data-dsh-sidebar-dragging] .vCHcua_panel,body[data-dsh-sidebar-dragging] .vCHcua_bottomPanel{will-change:transform}.vCHcua_bottomResize{cursor:row-resize;z-index:2;touch-action:none;height:8px;position:absolute;top:-4px;left:0;right:0}.vCHcua_bottomResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.vCHcua_bottomClose{z-index:4;width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:3px;right:6px}.vCHcua_bottomClose:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_bottomPanel .vCHcua_tabBar{padding-right:40px}body[data-dsh-title-bar-compat] .vCHcua_toggleCluster{top:calc(var(--dsh-title-bar-strip,40px) + 3px)}body[data-dsh-title-bar-compat] .vCHcua_panel{padding-top:var(--dsh-title-bar-strip,40px)}.vCHcua_cornerHandle{left:-6px;bottom:calc(var(--dsh-sidebar-height,0px) + 6px);z-index:2;cursor:nwse-resize;touch-action:none;width:12px;height:12px;position:absolute}.vCHcua_cornerHandle:hover,.vCHcua_cornerHandle[data-dragging]{background:var(--dsw-alias-interactive-bg-hover-accent)}.vCHcua_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.vCHcua_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_iconButton:disabled{opacity:.4;cursor:default}.vCHcua_workbench,.vCHcua_split{flex:1;min-width:0;min-height:0;display:flex}.vCHcua_splitRow{flex-direction:row}.vCHcua_splitCol{flex-direction:column}.vCHcua_splitChild{display:flex;position:relative;overflow:hidden}.vCHcua_divider{z-index:3;touch-action:none;flex:none;position:relative}.vCHcua_dividerRow:after,.vCHcua_dividerCol:after{content:\"\";background:var(--dsw-alias-border-l2);transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);position:absolute}.vCHcua_dividerRow{cursor:col-resize;width:7px;margin:0 -2px}.vCHcua_dividerRow:after{width:1px;top:0;bottom:0;left:50%;transform:translate(-50%)}.vCHcua_dividerCol{cursor:row-resize;height:7px;margin:-2px 0}.vCHcua_dividerCol:after{height:1px;top:50%;left:0;right:0;transform:translateY(-50%)}.vCHcua_divider:hover:after,.vCHcua_dividerActive:after{background:var(--dsw-alias-interactive-bg-hover-accent)}.vCHcua_pane{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}.vCHcua_paneDrop{outline:1px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.vCHcua_dropOverlay{z-index:6;pointer-events:none;background:var(--dsw-alias-interactive-bg-hover-accent);opacity:.5;position:absolute}.vCHcua_dropLeft{width:25%;top:0;bottom:0;left:0}.vCHcua_dropRight{width:25%;top:0;bottom:0;right:0}.vCHcua_dropUp{height:25%;top:0;left:0;right:0}.vCHcua_dropDown{height:25%;bottom:0;left:0;right:0}.vCHcua_dropCenter{outline:2px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-2px;background:0 0;inset:25%}.vCHcua_paneContent{flex-direction:column;flex:1;min-height:0;display:flex;overflow:hidden}.vCHcua_paneTab{flex-direction:column;flex:1;min-height:0;display:flex}.vCHcua_paneTab:has([aria-label$=\\.xlsx i]){isolation:isolate;contain:paint;position:relative}.vCHcua_paneTabHidden{display:none}.vCHcua_paneEmptyCards{flex:1;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));align-content:start;gap:8px;min-height:0;padding:12px;display:grid;overflow:hidden}.vCHcua_paneCard{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;text-align:center;border-radius:8px;flex-direction:column;justify-content:center;align-items:center;gap:6px;padding:12px 8px;display:flex}.vCHcua_paneCard:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2)}.vCHcua_paneCard:disabled{opacity:.45;cursor:default}.vCHcua_tabBar{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:stretch;height:34px;display:flex}.vCHcua_tabBarDrop{outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.vCHcua_tabList{scrollbar-width:none;flex:1;min-width:0;display:flex;overflow-x:auto}.vCHcua_tabList::-webkit-scrollbar{display:none}.vCHcua_tab{min-width:64px;max-width:160px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);border-right:1px solid var(--dsw-alias-border-l1);cursor:pointer;user-select:none;background:0 0;flex:none;align-items:center;gap:4px;padding:0 4px 0 10px;display:flex}.vCHcua_tab:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_tabActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.vCHcua_tabTitle{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.vCHcua_tabBadge{min-width:16px;height:15px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-brand-primary);border-radius:8px;flex:none;justify-content:center;align-items:center;padding:0 4px;display:inline-flex}.vCHcua_tabClose{width:18px;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.vCHcua_tabClose:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_tabBarPlus{background:var(--dsw-alias-bg-layer-1);width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;border:none;border-radius:5px;flex:none;justify-content:center;align-self:center;align-items:center;margin:0 6px;padding:0;display:inline-flex;position:sticky;right:0}.vCHcua_tabBarPlus:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_explorer{flex-direction:column;flex:1;min-height:0;display:flex}.vCHcua_explorerHeader{flex:none;justify-content:space-between;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.vCHcua_explorerRoot{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.vCHcua_explorerBody{flex:1;min-height:0;padding:2px 6px 8px;overflow:hidden auto}.vCHcua_explorerRow{box-sizing:border-box;width:100%;max-width:100%;height:34px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;white-space:nowrap;animation:vCHcua_dsh-row-in .15s var(--ds-ease-in-out);background:0 0;border:none;border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}.vCHcua_explorerRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_explorerDir{font:var(--dsw-font-s-strong-14)}.vCHcua_explorerHidden{opacity:.45}.vCHcua_explorerSymlink{color:var(--dsw-alias-label-tertiary);flex:none}.vCHcua_explorerBroken .vCHcua_explorerName{color:var(--dsw-alias-state-error-primary)}.vCHcua_explorerName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}.vCHcua_explorerRef{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;padding:0 8px;display:none}.vCHcua_explorerRef:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_explorerRow:hover .vCHcua_explorerRef,.vCHcua_explorerRow:focus-within .vCHcua_explorerRef{display:inline-flex}.vCHcua_explorerCopied{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.vCHcua_explorerError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);cursor:default}@keyframes vCHcua_dsh-row-in{0%{opacity:0}}.vCHcua_explorerEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.vCHcua_editor{flex-direction:column;flex:1;min-height:0;display:flex}.vCHcua_editorHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:6px;padding:4px 8px;display:flex}.vCHcua_editorTitle{min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.vCHcua_editorPathInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.vCHcua_editorPathInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.vCHcua_editorTreeToggleActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.vCHcua_editorBody{flex:1;min-height:0;display:flex}.vCHcua_editorMain{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.vCHcua_editorTreeDock{border-left:1px solid var(--dsw-alias-border-l1);flex:none;min-height:0;display:flex;position:relative}.vCHcua_editorTreeResize{cursor:col-resize;touch-action:none;z-index:3;width:6px;position:absolute;top:0;bottom:0;left:0}.vCHcua_editorTreeResize:hover{background:var(--dsw-alias-border-l2)}.vCHcua_editorTreePanel{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.vCHcua_editorTreePanelFull{flex:1}.vCHcua_editorTreeSearch{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.vCHcua_editorSearchInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.vCHcua_editorSearchInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.vCHcua_editorSearchHint{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:8px 12px}.vCHcua_editorSearchResult{width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);text-align:left;cursor:pointer;text-overflow:ellipsis;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 8px;display:block;overflow:hidden}.vCHcua_editorSearchResult:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_editorStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.vCHcua_editorStatusError{color:var(--dsw-alias-state-error-primary)}.vCHcua_dirtyDot{background:var(--dsw-alias-state-warn-primary);border-radius:50%;flex:none;width:7px;height:7px}.vCHcua_editorPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex:1;justify-content:center;align-items:center;padding:16px;display:flex}.vCHcua_orphanedType{opacity:.7;overflow-wrap:anywhere;margin-top:8px;font-size:12px;display:block}.vCHcua_editorBinary{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:12px;padding:24px 16px;display:flex}.vCHcua_editorBinaryNotice{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.vCHcua_editorDownloadLink{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), border-color var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:6px;align-items:center;gap:6px;padding:6px 14px;text-decoration:none;display:inline-flex}.vCHcua_editorDownloadLink:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}.vCHcua_editorError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);padding:12px 16px}.vCHcua_editorBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.vCHcua_sandboxStatus{font:var(--dsw-font-xxxs-11);flex:none;align-items:center;gap:8px;padding:4px 10px;display:flex}.vCHcua_sandboxStatusOn{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border-bottom:1px solid var(--dsw-alias-border-l1)}.vCHcua_sandboxStatusOff{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent);border-bottom:1px solid color-mix(in srgb, var(--dsw-alias-state-error-primary) 45%, transparent)}.vCHcua_sandboxDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:6px;height:6px}.vCHcua_sandboxStatusOff .vCHcua_sandboxDot{background:var(--dsw-alias-state-error-primary)}.vCHcua_sandboxStatusText{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.vCHcua_sandboxAction{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:6px;flex:none;padding:2px 8px}.vCHcua_sandboxAction:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_editorHtml{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.vCHcua_browser{flex-direction:column;flex:1;min-height:0;display:flex}.vCHcua_browserBar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.vCHcua_browserInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.vCHcua_browserInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.vCHcua_browserMessage{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.vCHcua_browserFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.vCHcua_browserStart{text-align:center;min-height:0;font:var(--dsw-font-xs-13);color:var(--dsw-alias-label-tertiary);flex:1;justify-content:center;align-items:center;padding:20px;display:flex}.vCHcua_browserBlocked{text-align:center;min-height:0;color:var(--dsw-alias-state-warn-primary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:6px;padding:24px;display:flex}.vCHcua_browserBlockedTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary)}.vCHcua_browserBlockedDesc{max-width:280px;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary)}.vCHcua_browserBlockedActions{gap:8px;margin-top:6px;display:flex}.vCHcua_browserBlockedButton{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-11);cursor:pointer;border-radius:6px;padding:4px 12px}.vCHcua_browserBlockedButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_editorCm{background:0 0;flex:1;min-height:0;overflow:hidden}.vCHcua_editorCmHidden{display:none}.vCHcua_editorCm .cm-editor{height:100%}.vCHcua_editorCm .cm-editor.cm-focused{outline:none}.vCHcua_editorModeToggle{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;flex:none;align-items:center;gap:2px;padding:2px;display:inline-flex}.vCHcua_editorModeButton{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;padding:2px 8px}.vCHcua_editorModeButton:hover{color:var(--dsw-alias-label-primary)}.vCHcua_editorModeActive{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.vCHcua_editorImageWrap{flex:1;justify-content:center;align-items:center;min-height:0;padding:12px;display:flex;overflow:auto}.vCHcua_editorImage{object-fit:contain;max-width:100%;max-height:100%}.vCHcua_editorMd{min-height:0;font:var(--dsw-font-xs-13);flex:1;padding:10px 14px;overflow-y:auto}.vCHcua_mermaidWrap{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;margin:6px 0;overflow:hidden}.vCHcua_mermaidHeader{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);justify-content:space-between;align-items:center;gap:6px;padding:4px 8px;display:flex}.vCHcua_mermaidInfo{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary)}.vCHcua_mermaidCopy{height:20px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;align-items:center;gap:4px;padding:0 6px;display:inline-flex}.vCHcua_mermaidCopy:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_mermaidBody{cursor:zoom-in;justify-content:center;padding:10px;display:flex;overflow:auto}.vCHcua_mermaidBody svg{max-width:100%;height:auto}.vCHcua_mermaidError{border-bottom:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-11);padding:6px 10px}.vCHcua_mermaidCode{font:var(--dsw-font-xxxs-11);margin:0;padding:8px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto}.vCHcua_mermaidMarkdown .md-code-block[data-mermaid-processed]{display:contents}.vCHcua_mermaidModal{z-index:1000;background:var(--dsw-alias-bg-mask-1);backdrop-filter:blur(2px);flex-direction:column;justify-content:center;align-items:center;display:flex;position:fixed;inset:0}.vCHcua_mermaidModalToolbar{z-index:10;gap:8px;display:flex;position:absolute;top:16px;right:16px}.vCHcua_mermaidModalButton{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);width:36px;height:36px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xs-strong-13);cursor:pointer;border-radius:8px;justify-content:center;align-items:center;display:inline-flex}.vCHcua_mermaidModalButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_mermaidModalStage{justify-content:center;align-items:center;width:90vw;height:80vh;display:flex;position:relative;overflow:hidden}.vCHcua_mermaidModalStage svg{cursor:grab;transform-origin:50%;user-select:none;-webkit-user-drag:none;background:var(--dsw-alias-bg-layer-1);border-radius:12px;max-width:none;max-height:none;padding:16px}.vCHcua_mermaidModalStage svg:active{cursor:grabbing}.vCHcua_mermaidModalHint{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);pointer-events:none;position:absolute;bottom:16px;left:50%;transform:translate(-50%)}.vCHcua_selectionPopup{z-index:60;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;cursor:pointer;border-radius:6px;align-items:center;padding:0 10px;display:inline-flex;position:fixed;transform:translate(-50%,calc(-100% - 8px))}.vCHcua_selectionPopup:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_editorPdf{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex}.vCHcua_editorPdfToolbar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;justify-content:flex-end;padding:6px 8px;display:flex}.vCHcua_editorPdfStage{flex:1;min-height:0;display:flex;position:relative}.vCHcua_editorPdfFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.vCHcua_editorPdfFrameBlocked{pointer-events:none}.vCHcua_editorPdfDragShield{z-index:4;pointer-events:none;background:0 0;position:absolute;inset:0}.vCHcua_editorPdfDragShieldActive{pointer-events:auto}body[data-dsh-tab-dragging] .vCHcua_editorPdfFrame{pointer-events:none!important}body[data-dsh-tab-dragging] .vCHcua_editorPdfDragShield{pointer-events:auto!important}.vCHcua_terminalWrap{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex;position:relative}.vCHcua_terminal{flex:1;min-height:0;padding:6px 4px 6px 8px}.vCHcua_terminal .xterm{height:100%}.vCHcua_terminalBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-wrap:wrap;flex:none;align-items:center;gap:8px;padding:3px 10px;display:flex}.vCHcua_terminalBannerUrl{word-break:break-all;opacity:.85;flex-basis:100%;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.vCHcua_boundaryError{z-index:50;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;align-items:flex-start;gap:8px;padding:16px;display:flex;position:fixed;top:0;bottom:0;right:0;overflow:auto}.vCHcua_terminalRetry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;padding:1px 8px}.vCHcua_terminalRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_terminalDepsBanner{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-direction:column;flex:none;gap:6px;padding:10px;display:flex}.vCHcua_terminalDepsTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-state-warn-primary)}.vCHcua_terminalDepsHint{opacity:.9}.vCHcua_terminalDepsCommandRow{align-items:flex-start;gap:8px;display:flex}.vCHcua_terminalRepairCommand{white-space:pre-wrap;word-break:break-all;user-select:text;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:4px;flex:1;max-height:160px;margin:0;padding:6px 8px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:1.5;overflow:auto}.vCHcua_terminalDepsNote{opacity:.85}.vCHcua_terminalDepsActions{align-items:center;gap:8px;display:flex}.vCHcua_tabBoundaryError{min-height:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;flex:1;align-items:flex-start;gap:8px;padding:12px 16px;display:flex;overflow:auto}.vCHcua_git{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.vCHcua_gitHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.vCHcua_gitBranchSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 6px}.vCHcua_gitSection{border-top:1px solid var(--dsw-alias-border-l1)}.vCHcua_gitSectionHeader{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);text-transform:uppercase;justify-content:space-between;align-items:center;padding:6px 12px 4px;display:flex}.vCHcua_gitLink{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-brand-primary);cursor:pointer;background:0 0;border:none;padding:0}.vCHcua_gitLink:hover:not(:disabled){text-decoration:underline}.vCHcua_gitLink:disabled{opacity:.4;cursor:default}.vCHcua_gitRow{min-height:34px;animation:vCHcua_dsh-row-in .15s var(--ds-ease-in-out);border-radius:8px;align-items:center;gap:6px;margin:0 6px;padding:0 8px;display:flex}.vCHcua_gitRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_gitRowSelected{background:var(--dsw-alias-interactive-bg-active)}.vCHcua_gitRowMain{cursor:pointer;text-align:left;background:0 0;border:none;flex:1;align-items:center;gap:8px;min-width:0;padding:3px 0;display:flex}.vCHcua_gitBadge{width:20px;height:16px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;justify-content:center;align-items:center;display:inline-flex}.vCHcua_gitName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.vCHcua_gitEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:4px 12px 8px}.vCHcua_gitPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.vCHcua_gitError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;padding:8px 12px}.vCHcua_gitDiff{border-top:1px solid var(--dsw-alias-border-l1);padding:8px}.vCHcua_gitDiffTab{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.vCHcua_gitDiffTabHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.vCHcua_gitDiffTabTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.vCHcua_gitDiffFile{align-items:baseline;gap:6px;padding:8px 2px 2px;display:flex}.vCHcua_gitDiffFilePath{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.vCHcua_gitDiffFileOld{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}.vCHcua_gitDiffFileTag{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:0 6px}.vCHcua_gitDiffHunk{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);gap:8px;padding:3px 2px;display:flex}.vCHcua_gitDiffHunkHeader{color:var(--dsw-alias-label-secondary);flex:none}.vCHcua_gitDiffHunkSection{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.vCHcua_gitDiffLine{font:var(--dsw-font-markdown-code-block-small);white-space:pre-wrap;overflow-wrap:anywhere;align-items:stretch;min-width:0;line-height:20px;display:flex}.vCHcua_gitDiffNum{text-align:right;width:36px;color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;padding-right:8px}.vCHcua_gitDiffCode{flex:1;min-width:0;overflow:visible}.vCHcua_gitDiffCtx{color:var(--dsw-alias-label-primary)}.vCHcua_gitDiffDel{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent)}.vCHcua_gitDiffAdd{color:var(--dsw-alias-state-success-primary);background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent)}.vCHcua_gitDiffMeta{padding-left:2px}.vCHcua_gitDiffMetaText{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);font-style:italic}.vCHcua_gitDiffExpand{width:100%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-brand-primary);cursor:pointer;text-align:center;background:0 0;border:none;margin:4px 0;display:block}.vCHcua_gitDiffExpand:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_gitConfirmDesc{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);white-space:pre-wrap;margin:0}.vCHcua_gitCommit{border-top:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;padding:8px 12px;display:flex}.vCHcua_gitCommitInput{flex:1;min-width:0}.vCHcua_gitCommitButton{background:var(--dsw-alias-button-primary-fill);height:26px;color:var(--dsw-alias-label-primary-inverted);font:var(--dsw-font-xxs-strong-12);cursor:pointer;border:none;border-radius:6px;flex:none;padding:0 12px}.vCHcua_gitCommitButton:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}.vCHcua_gitCommitButton:disabled{opacity:.45;cursor:default}.vCHcua_gitLogRow{cursor:pointer;border-radius:8px;flex-direction:column;gap:2px;padding:5px 12px;display:flex}.vCHcua_gitLogRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.vCHcua_gitLogLine1{align-items:baseline;gap:8px;min-width:0;display:flex}.vCHcua_gitLogHash{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);flex:none}.vCHcua_gitLogLine2{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;display:flex}.vCHcua_gitLogRef{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-brand-primary);white-space:nowrap;border-radius:999px;flex:none;padding:0 5px}.vCHcua_gitLogSubject{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.vCHcua_gitLogMeta{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.vCHcua_gitLogMore{border:1px solid var(--dsw-alias-border-l2);width:calc(100% - 24px);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;margin:4px 12px 8px;padding:6px 0;display:block}.vCHcua_gitLogMore:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_gitLogMore:disabled{opacity:.5;cursor:default}.vCHcua_producedRow{flex-wrap:wrap;align-items:center;gap:8px;padding:4px 0;display:flex}.vCHcua_producedLabel{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.vCHcua_producedChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:200px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;display:inline-flex;overflow:hidden}.vCHcua_producedChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.vCHcua_producedChip span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.vCHcua_producedMore{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.vCHcua_producedFolder{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxs-12);cursor:pointer;background:0 0;border:0;border-radius:4px;padding:0 2px}.vCHcua_producedFolder:hover{color:var(--dsw-alias-label-secondary);text-decoration:underline}.vCHcua_toggleButton:focus-visible,.vCHcua_bottomClose:focus-visible,.vCHcua_iconButton:focus-visible,.vCHcua_tab:focus-visible,.vCHcua_tabClose:focus-visible,.vCHcua_tabBarPlus:focus-visible,.vCHcua_paneCard:focus-visible,.vCHcua_explorerRow:focus-visible,.vCHcua_explorerRef:focus-visible,.vCHcua_gitRowMain:focus-visible,.vCHcua_gitLink:focus-visible,.vCHcua_gitCommitButton:focus-visible,.vCHcua_gitLogRow:focus-visible,.vCHcua_gitLogMore:focus-visible,.vCHcua_gitDiffExpand:focus-visible,.vCHcua_terminalRetry:focus-visible,.vCHcua_editorModeButton:focus-visible,.vCHcua_editorDownloadLink:focus-visible,.vCHcua_editorPptxButton:focus-visible,.vCHcua_editorDocxZoomRange:focus-visible{outline:2px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}@media (prefers-reduced-motion:reduce){.vCHcua_panel,.vCHcua_panelHidden,.vCHcua_bottomPanel,.vCHcua_bottomPanelHidden,.vCHcua_toggleCluster,.vCHcua_toggleButton,.vCHcua_tab,.vCHcua_tabBarPlus,.vCHcua_paneCard,.vCHcua_explorerRow,.vCHcua_gitRow,.vCHcua_divider,.vCHcua_dividerRow:after,.vCHcua_dividerCol:after{transition:none;animation:none}}@media (width<=767px){.vCHcua_panel:not(.vCHcua_panelHidden) .vCHcua_tabBar{padding-right:40px}.vCHcua_tab{min-width:48px;max-width:128px}}";
		...
		var sidebar_module_css_default = {
			"bottomClose": "vCHcua_bottomClose",
			"bottomPanel": "vCHcua_bottomPanel",
			"bottomPanelHidden": "vCHcua_bottomPanelHidden",
			"bottomResize": "vCHcua_bottomResize",
			"bottomResizeActive": "vCHcua_bottomResizeActive",
			"boundaryError": "vCHcua_boundaryError",
			"browser": "vCHcua_browser",
			"browserBar": "vCHcua_browserBar",
			"browserBlocked": "vCHcua_browserBlocked",
			"browserBlockedActions": "vCHcua_browserBlockedActions",
			"browserBlockedButton": "vCHcua_browserBlockedButton",
			"browserBlockedDesc": "vCHcua_browserBlockedDesc",
			"browserBlockedTitle": "vCHcua_browserBlockedTitle",
			"browserFrame": "vCHcua_browserFrame",
			"browserInput": "vCHcua_browserInput",
			"browserMessage": "vCHcua_browserMessage",
			"browserStart": "vCHcua_browserStart",
			"cornerHandle": "vCHcua_cornerHandle",
			"dirtyDot": "vCHcua_dirtyDot",
			"divider": "vCHcua_divider",
			"dividerActive": "vCHcua_dividerActive",
			"dividerCol": "vCHcua_dividerCol",
			"dividerRow": "vCHcua_dividerRow",
			"dropCenter": "vCHcua_dropCenter",
			"dropDown": "vCHcua_dropDown",
			"dropLeft": "vCHcua_dropLeft",
			"dropOverlay": "vCHcua_dropOverlay",
			"dropRight": "vCHcua_dropRight",
			"dropUp": "vCHcua_dropUp",
			"dsh-row-in": "vCHcua_dsh-row-in",
			"editor": "vCHcua_editor",
			"editorBanner": "vCHcua_editorBanner",
			"editorBinary": "vCHcua_editorBinary",
			"editorBinaryNotice": "vCHcua_editorBinaryNotice",
			"editorBody": "vCHcua_editorBody",
			"editorCm": "vCHcua_editorCm",
			"editorCmHidden": "vCHcua_editorCmHidden",
			"editorDocxZoomRange": "vCHcua_editorDocxZoomRange",
			"editorDownloadLink": "vCHcua_editorDownloadLink",
			"editorError": "vCHcua_editorError",
			"editorHeader": "vCHcua_editorHeader",
			"editorHtml": "vCHcua_editorHtml",
			"editorImage": "vCHcua_editorImage",
			"editorImageWrap": "vCHcua_editorImageWrap",
			"editorMain": "vCHcua_editorMain",
			"editorMd": "vCHcua_editorMd",
			"editorModeActive": "vCHcua_editorModeActive",
			"editorModeButton": "vCHcua_editorModeButton",
			"editorModeToggle": "vCHcua_editorModeToggle",
			"editorPathInput": "vCHcua_editorPathInput",
			"editorPdf": "vCHcua_editorPdf",
			"editorPdfDragShield": "vCHcua_editorPdfDragShield",
			"editorPdfDragShieldActive": "vCHcua_editorPdfDragShieldActive",
			"editorPdfFrame": "vCHcua_editorPdfFrame",
			"editorPdfFrameBlocked": "vCHcua_editorPdfFrameBlocked",
			"editorPdfStage": "vCHcua_editorPdfStage",
			"editorPdfToolbar": "vCHcua_editorPdfToolbar",
			"editorPlaceholder": "vCHcua_editorPlaceholder",
			"editorPptxButton": "vCHcua_editorPptxButton",
			"editorSearchHint": "vCHcua_editorSearchHint",
			"editorSearchInput": "vCHcua_editorSearchInput",
			"editorSearchResult": "vCHcua_editorSearchResult",
			"editorStatus": "vCHcua_editorStatus",
			"editorStatusError": "vCHcua_editorStatusError",
			"editorTitle": "vCHcua_editorTitle",
			"editorTreeDock": "vCHcua_editorTreeDock",
			"editorTreePanel": "vCHcua_editorTreePanel",
			"editorTreePanelFull": "vCHcua_editorTreePanelFull",
			"editorTreeResize": "vCHcua_editorTreeResize",
			"editorTreeSearch": "vCHcua_editorTreeSearch",
			"editorTreeToggleActive": "vCHcua_editorTreeToggleActive",
			"explorer": "vCHcua_explorer",
			"explorerBody": "vCHcua_explorerBody",
			"explorerBroken": "vCHcua_explorerBroken",
			"explorerCopied": "vCHcua_explorerCopied",
			"explorerDir": "vCHcua_explorerDir",
			"explorerEmpty": "vCHcua_explorerEmpty",
			"explorerError": "vCHcua_explorerError",
			"explorerHeader": "vCHcua_explorerHeader",
			"explorerHidden": "vCHcua_explorerHidden",
			"explorerName": "vCHcua_explorerName",
			"explorerRef": "vCHcua_explorerRef",
			"explorerRoot": "vCHcua_explorerRoot",
			"explorerRow": "vCHcua_explorerRow",
			"explorerSymlink": "vCHcua_explorerSymlink",
			"git": "vCHcua_git",
			"gitBadge": "vCHcua_gitBadge",
			"gitBranchSelect": "vCHcua_gitBranchSelect",
			"gitCommit": "vCHcua_gitCommit",
			"gitCommitButton": "vCHcua_gitCommitButton",
			"gitCommitInput": "vCHcua_gitCommitInput",
			"gitConfirmDesc": "vCHcua_gitConfirmDesc",
			"gitDiff": "vCHcua_gitDiff",
			"gitDiffAdd": "vCHcua_gitDiffAdd",
			"gitDiffCode": "vCHcua_gitDiffCode",
			"gitDiffCtx": "vCHcua_gitDiffCtx",
			"gitDiffDel": "vCHcua_gitDiffDel",
			"gitDiffExpand": "vCHcua_gitDiffExpand",
			"gitDiffFile": "vCHcua_gitDiffFile",
			"gitDiffFileOld": "vCHcua_gitDiffFileOld",
			"gitDiffFilePath": "vCHcua_gitDiffFilePath",
			"gitDiffFileTag": "vCHcua_gitDiffFileTag",
			"gitDiffHunk": "vCHcua_gitDiffHunk",
			"gitDiffHunkHeader": "vCHcua_gitDiffHunkHeader",
			"gitDiffHunkSection": "vCHcua_gitDiffHunkSection",
			"gitDiffLine": "vCHcua_gitDiffLine",
			"gitDiffMeta": "vCHcua_gitDiffMeta",
			"gitDiffMetaText": "vCHcua_gitDiffMetaText",
			"gitDiffNum": "vCHcua_gitDiffNum",
			"gitDiffTab": "vCHcua_gitDiffTab",
			"gitDiffTabHeader": "vCHcua_gitDiffTabHeader",
			"gitDiffTabTitle": "vCHcua_gitDiffTabTitle",
			"gitEmpty": "vCHcua_gitEmpty",
			"gitError": "vCHcua_gitError",
			"gitHeader": "vCHcua_gitHeader",
			"gitLink": "vCHcua_gitLink",
			"gitLogHash": "vCHcua_gitLogHash",
			"gitLogLine1": "vCHcua_gitLogLine1",
			"gitLogLine2": "vCHcua_gitLogLine2",
			"gitLogMeta": "vCHcua_gitLogMeta",
			"gitLogMore": "vCHcua_gitLogMore",
			"gitLogRef": "vCHcua_gitLogRef",
			"gitLogRow": "vCHcua_gitLogRow",
			"gitLogSubject": "vCHcua_gitLogSubject",
			"gitName": "vCHcua_gitName",
			"gitPlaceholder": "vCHcua_gitPlaceholder",
			"gitRow": "vCHcua_gitRow",
			"gitRowMain": "vCHcua_gitRowMain",
			"gitRowSelected": "vCHcua_gitRowSelected",
			"gitSection": "vCHcua_gitSection",
			"gitSectionHeader": "vCHcua_gitSectionHeader",
			"iconButton": "vCHcua_iconButton",
			"mermaidBody": "vCHcua_mermaidBody",
			"mermaidCode": "vCHcua_mermaidCode",
			"mermaidCopy": "vCHcua_mermaidCopy",
			"mermaidError": "vCHcua_mermaidError",
			"mermaidHeader": "vCHcua_mermaidHeader",
			"mermaidInfo": "vCHcua_mermaidInfo",
			"mermaidMarkdown": "vCHcua_mermaidMarkdown",
			"mermaidModal": "vCHcua_mermaidModal",
			"mermaidModalButton": "vCHcua_mermaidModalButton",
			"mermaidModalHint": "vCHcua_mermaidModalHint",
			"mermaidModalStage": "vCHcua_mermaidModalStage",
			"mermaidModalToolbar": "vCHcua_mermaidModalToolbar",
			"mermaidWrap": "vCHcua_mermaidWrap",
			"orphanedType": "vCHcua_orphanedType",
			"pane": "vCHcua_pane",
			"paneCard": "vCHcua_paneCard",
			"paneContent": "vCHcua_paneContent",
			"paneDrop": "vCHcua_paneDrop",
			"paneEmptyCards": "vCHcua_paneEmptyCards",
			"panel": "vCHcua_panel",
			"panelBody": "vCHcua_panelBody",
			"panelHidden": "vCHcua_panelHidden",
			"panelResize": "vCHcua_panelResize",
			"panelResizeActive": "vCHcua_panelResizeActive",
			"paneTab": "vCHcua_paneTab",
			"paneTabHidden": "vCHcua_paneTabHidden",
			"producedChip": "vCHcua_producedChip",
			"producedFolder": "vCHcua_producedFolder",
			"producedLabel": "vCHcua_producedLabel",
			"producedMore": "vCHcua_producedMore",
			"producedRow": "vCHcua_producedRow",
			"sandboxAction": "vCHcua_sandboxAction",
			"sandboxDot": "vCHcua_sandboxDot",
			"sandboxStatus": "vCHcua_sandboxStatus",
			"sandboxStatusOff": "vCHcua_sandboxStatusOff",
			"sandboxStatusOn": "vCHcua_sandboxStatusOn",
			"sandboxStatusText": "vCHcua_sandboxStatusText",
			"selectionPopup": "vCHcua_selectionPopup",
			"split": "vCHcua_split",
			"splitChild": "vCHcua_splitChild",
			"splitCol": "vCHcua_splitCol",
			"splitRow": "vCHcua_splitRow",
			"tab": "vCHcua_tab",
			"tabActive": "vCHcua_tabActive",
			"tabBadge": "vCHcua_tabBadge",
			"tabBar": "vCHcua_tabBar",
			"tabBarDrop": "vCHcua_tabBarDrop",
			"tabBarPlus": "vCHcua_tabBarPlus",
			"tabBoundaryError": "vCHcua_tabBoundaryError",
			"tabClose": "vCHcua_tabClose",
			"tabList": "vCHcua_tabList",
			"tabTitle": "vCHcua_tabTitle",
			"terminal": "vCHcua_terminal",
			"terminalBanner": "vCHcua_terminalBanner",
			"terminalBannerUrl": "vCHcua_terminalBannerUrl",
			"terminalDepsActions": "vCHcua_terminalDepsActions",
			"terminalDepsBanner": "vCHcua_terminalDepsBanner",
			"terminalDepsCommandRow": "vCHcua_terminalDepsCommandRow",
			"terminalDepsHint": "vCHcua_terminalDepsHint",
			"terminalDepsNote": "vCHcua_terminalDepsNote",
			"terminalDepsTitle": "vCHcua_terminalDepsTitle",
			"terminalRepairCommand": "vCHcua_terminalRepairCommand",
			"terminalRetry": "vCHcua_terminalRetry",
			"terminalWrap": "vCHcua_terminalWrap",
			"toggleButton": "vCHcua_toggleButton",
			"toggleCluster": "vCHcua_toggleCluster",
			"workbench": "vCHcua_workbench"
		};
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/intercept.tsx
		/**
		...
		*/
		...
		function SidebarProducedFiles(props) {
			const { matched, openInSidebar, openFolder } = props;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					hidden > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: sidebar_module_css_default.producedFolder,
						onClick: openFolder,
						children: t("producedShowFolder")
					})
				]
			});
		}
			...
			return ctx.slots.inject("conversation.chat.turnTail", () => ctx.slots.register({
				...
				inject: (sessionId) => ({
					...
					openFolder: () => {
						...
						ctx.workspaces.openPath(resolveSidebarPath(cwd, "."));
					}
				})
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/api.ts
		/** One wire failure. */
		...
		*  `Content-Disposition: attachment`, so the browser saves the file. */
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/binary-download.tsx
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/editor-load.ts
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/FileTree.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/tree-refresh.ts
		/** Refresh cadence while the Files panel is visible. */
		const TREE_AUTO_REFRESH_MS = 2e3;
		/**
		...
		*/
		function subscribeTreeRefresh(refresh, intervalMs = TREE_AUTO_REFRESH_MS) {
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/TreePanel.tsx
		/**
		...
		*/
		function TreePanel(props) {
			...
			(0, react.useEffect)(() => subscribeTreeRefresh(() => {
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/EditorHost.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/lazy-chunk.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/GitView.tsx
		/**
		...
		*  floods the panel at once (the end of the log is reached by paging). */
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/DiffView.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/DiffTab.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/subagent-detect.ts
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/subagent-activity.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/subagent-jobs.ts
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/icons.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\ChatGPT\RuijieDSH\vendor\dsh-better-sidebar\src\client\SubagentView.module.css.mjs
		const css$2 = ".y5rrmq_subagent{flex-direction:column;flex:1;min-height:0;display:flex}.y5rrmq_subagentHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.y5rrmq_subagentTitle{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.y5rrmq_subagentCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.y5rrmq_subagentRefresh{width:24px;height:24px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;display:inline-flex}.y5rrmq_subagentRefresh:hover{background:var(--dsw-alias-interactive-bg-hover)}.y5rrmq_subagentBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}.y5rrmq_subagentRow{box-sizing:border-box;width:100%;min-height:50px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;align-items:flex-start;gap:8px;padding:7px 8px 7px 11px;display:flex;position:relative}.y5rrmq_subagentRow:hover,.y5rrmq_subagentRow:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.y5rrmq_subagentRowActive,.y5rrmq_subagentRowActive:hover,.y5rrmq_subagentRowActive:focus-visible{background:var(--dsw-alias-interactive-bg-active)}.y5rrmq_subagentRowDisabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.y5rrmq_subagentRowDisabled:hover{background:0 0}.y5rrmq_subagentRowLoading{cursor:default}.y5rrmq_subagentDot{margin-top:4px}.y5rrmq_subagentContent{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.y5rrmq_subagentLabel,.y5rrmq_subagentSecondary{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.y5rrmq_subagentLabel{color:inherit;font-weight:400}.y5rrmq_subagentSecondary{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.y5rrmq_subagentLive{min-width:0;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);align-items:baseline;gap:4px;display:flex;overflow:hidden}.y5rrmq_subagentLiveTool{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);flex:none}.y5rrmq_subagentLiveArgs{min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.y5rrmq_subagentLiveText{-webkit-line-clamp:2;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary);-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.y5rrmq_subagentNode{min-width:0;position:relative}.y5rrmq_subagentChildren{margin-left:18px;padding-left:4px;position:relative}.y5rrmq_subagentChildren:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);height:26px;position:absolute;top:-26px;left:0}.y5rrmq_subagentChildren[aria-busy=true]:before{content:none}.y5rrmq_subagentChildren>.y5rrmq_subagentNode:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);position:absolute;top:0;bottom:0;left:-4px}.y5rrmq_subagentChildren>.y5rrmq_subagentNode:last-child:before{height:17px;bottom:auto}.y5rrmq_subagentChildren>.y5rrmq_subagentNode>.y5rrmq_subagentRow:before{content:\"\";border-top:1px solid var(--dsw-alias-border-l2);width:14px;position:absolute;top:16px;left:-4px}.y5rrmq_subagentEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;gap:2px;padding:16px;display:flex}.y5rrmq_subagentEmptyHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-dimmed)}.y5rrmq_subagentError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;display:flex}.y5rrmq_subagentErrorRetry{height:24px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;align-items:center;gap:4px;padding:0 8px;display:inline-flex}.y5rrmq_subagentErrorRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.y5rrmq_jobs{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:8px}.y5rrmq_jobsHeader{align-items:center;gap:8px;height:26px;padding:0 2px;display:flex}.y5rrmq_jobsTitle{min-width:0;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.y5rrmq_jobsCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.y5rrmq_jobsList{flex-direction:column;gap:2px;margin:0;padding:0;list-style:none;display:flex}.y5rrmq_jobsRow{border-radius:8px;align-items:center;gap:4px;display:flex}.y5rrmq_jobsRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.y5rrmq_jobsRowSettled{opacity:.8}.y5rrmq_jobsRowSelected,.y5rrmq_jobsRowSelected:hover{background:var(--dsw-alias-interactive-bg-active)}.y5rrmq_jobsRowMain{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;flex:1;align-items:flex-start;gap:8px;padding:6px 8px 6px 11px;display:flex}.y5rrmq_jobsRowMain:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.y5rrmq_jobsDot{margin-top:5px}.y5rrmq_jobsContent{flex-direction:column;gap:1px;min-width:0;display:flex}.y5rrmq_jobsLabelLine{align-items:center;gap:6px;min-width:0;display:flex}.y5rrmq_jobsKind{text-overflow:ellipsis;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);max-width:90px;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;padding:0 5px;line-height:14px;overflow:hidden}.y5rrmq_jobsLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.y5rrmq_jobsSecondary{text-overflow:ellipsis;white-space:nowrap;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);overflow:hidden}.y5rrmq_jobsKill{width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;margin-right:4px;display:inline-flex}.y5rrmq_jobsKill:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);color:var(--dsw-alias-state-error-primary)}.y5rrmq_jobsKillArmed,.y5rrmq_jobsKillArmed:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);width:auto;height:20px;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;padding:0 8px}.y5rrmq_jobsKill:disabled{opacity:.5;cursor:default}.y5rrmq_jobsKillError{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-error-primary);flex:none;margin-right:4px}.y5rrmq_jobsPane{z-index:1;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;margin-top:4px;position:sticky;bottom:0;overflow:hidden;box-shadow:0 -6px 12px -8px #00000059}.y5rrmq_jobsPaneHeader{border-bottom:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;display:flex}.y5rrmq_jobsPaneDot{flex:none}.y5rrmq_jobsPaneLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.y5rrmq_jobsPaneStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.y5rrmq_jobsPaneClose{width:20px;height:20px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:5px;flex:none;justify-content:center;align-items:center;display:inline-flex}.y5rrmq_jobsPaneClose:hover{background:var(--dsw-alias-interactive-bg-hover)}.y5rrmq_jobsPanePre{max-height:200px;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;margin:0;padding:6px 10px;line-height:1.5;overflow:auto}.y5rrmq_jobsPaneHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);padding:8px 10px}.y5rrmq_jobsPaneError{color:var(--dsw-alias-state-error-primary)}";
		...
		var SubagentView_module_css_default = {
			"jobs": "y5rrmq_jobs",
			"jobsContent": "y5rrmq_jobsContent",
			"jobsCount": "y5rrmq_jobsCount",
			"jobsDot": "y5rrmq_jobsDot",
			"jobsHeader": "y5rrmq_jobsHeader",
			"jobsKill": "y5rrmq_jobsKill",
			"jobsKillArmed": "y5rrmq_jobsKillArmed",
			"jobsKillError": "y5rrmq_jobsKillError",
			"jobsKind": "y5rrmq_jobsKind",
			"jobsLabel": "y5rrmq_jobsLabel",
			"jobsLabelLine": "y5rrmq_jobsLabelLine",
			"jobsList": "y5rrmq_jobsList",
			"jobsPane": "y5rrmq_jobsPane",
			"jobsPaneClose": "y5rrmq_jobsPaneClose",
			"jobsPaneDot": "y5rrmq_jobsPaneDot",
			"jobsPaneError": "y5rrmq_jobsPaneError",
			"jobsPaneHeader": "y5rrmq_jobsPaneHeader",
			"jobsPaneHint": "y5rrmq_jobsPaneHint",
			"jobsPaneLabel": "y5rrmq_jobsPaneLabel",
			"jobsPanePre": "y5rrmq_jobsPanePre",
			"jobsPaneStatus": "y5rrmq_jobsPaneStatus",
			"jobsRow": "y5rrmq_jobsRow",
			"jobsRowMain": "y5rrmq_jobsRowMain",
			"jobsRowSelected": "y5rrmq_jobsRowSelected",
			"jobsRowSettled": "y5rrmq_jobsRowSettled",
			"jobsSecondary": "y5rrmq_jobsSecondary",
			"jobsTitle": "y5rrmq_jobsTitle",
			"subagent": "y5rrmq_subagent",
			"subagentBody": "y5rrmq_subagentBody",
			"subagentChildren": "y5rrmq_subagentChildren",
			"subagentContent": "y5rrmq_subagentContent",
			"subagentCount": "y5rrmq_subagentCount",
			"subagentDot": "y5rrmq_subagentDot",
			"subagentEmpty": "y5rrmq_subagentEmpty",
			"subagentEmptyHint": "y5rrmq_subagentEmptyHint",
			"subagentError": "y5rrmq_subagentError",
			"subagentErrorRetry": "y5rrmq_subagentErrorRetry",
			"subagentHeader": "y5rrmq_subagentHeader",
			"subagentLabel": "y5rrmq_subagentLabel",
			"subagentLive": "y5rrmq_subagentLive",
			"subagentLiveArgs": "y5rrmq_subagentLiveArgs",
			"subagentLiveText": "y5rrmq_subagentLiveText",
			"subagentLiveTool": "y5rrmq_subagentLiveTool",
			"subagentNode": "y5rrmq_subagentNode",
			"subagentRefresh": "y5rrmq_subagentRefresh",
			"subagentRow": "y5rrmq_subagentRow",
			"subagentRowActive": "y5rrmq_subagentRowActive",
			"subagentRowDisabled": "y5rrmq_subagentRowDisabled",
			"subagentRowLoading": "y5rrmq_subagentRowLoading",
			"subagentSecondary": "y5rrmq_subagentSecondary",
			"subagentTitle": "y5rrmq_subagentTitle"
		};
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/SubagentView.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/browser.ts
		/**
		...
		*  they parse as hosts below. */
		...
		function normalizeBrowserUrl(input, selfOrigin) {
			...
			if (!(/^(?:https?:\/\/)/iu.test(trimmed) || /^\[[0-9a-f:]+\](?::\d+)?(?:\/.*)?$/iu.test(trimmed) || /^(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?(?:\/.*)?$/u.test(trimmed) || /^(?:localhost|[^\s./]+(?:\.[^\s./]+)+)(?::\d+)?(?:\/.*)?$/iu.test(trimmed)) && !/^[a-zA-Z][a-zA-Z0-9+.-]*:/u.test(trimmed)) return {
				kind: "ok",
				url: `https://cn.bing.com/search?q=${encodeURIComponent(trimmed)}`
			};
		...
		/**
		* Recover a useful search when the current corporate network cannot reach a
		* Baidu page. Other hosts return undefined and keep their normal error UI.
		*/
		function fallbackSearchUrlForBlockedSite(input) {
			let url;
			try {
				url = new URL(input);
			} catch {
			...
			if (hostname !== "baidu.com" && !hostname.endsWith(".baidu.com")) return void 0;
			let query = url.searchParams.get("wd") ?? url.searchParams.get("word") ?? "";
			if (hostname === "baike.baidu.com") {
				const segments = url.pathname.split("/").filter(Boolean);
				const item = segments.findIndex((segment) => segment.toLowerCase() === "item");
				const topic = item >= 0 ? segments[item + 1] : segments[0];
				if (topic !== void 0) try {
					query = decodeURIComponent(topic).replaceAll("_", " ");
				} catch {
					query = topic.replaceAll("_", " ");
				}
			}
			if (query.trim() === "") query = url.href;
			return `https://cn.bing.com/search?q=${encodeURIComponent(query.trim())}`;
		}
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/SandboxStatusBar.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/BrowserView.tsx
		/**
  ...
  return { url: location.href, title: clean(document.title), text: best.slice(0, limit), links, truncated: best.length > limit };
		...
		*/
		...
		function BrowserView(props) {
			...
			const fallbackAttemptsRef = (0, react.useRef)(/* @__PURE__ */ new Set());
			...
			*  the probe verdict shown instead of the blank iframe. */
			...
			const [nativeHistory, setNativeHistory] = (0, react.useState)({
				back: false,
				forward: false
			});
			...
			(0, react.useEffect)(() => {
				if (!nativeWebview || url === void 0) return;
				...
				const failed = (event) => {
					...
					if (fallback !== void 0 && !fallbackAttemptsRef.current.has(target)) {
				...
				const unsubscribePopup = window.__DSH_DESKTOP_SIDEBAR_POPUP__?.onNavigate((guestId, nextUrl) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					!nativeWebview && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SandboxStatusBar, {
						sandboxed: !noSandbox,
						local: localUnlock,
					...
					}) : nativeWebview ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("webview", {
						ref: (node) => {
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/builtins/tabs.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/PdfView.tsx
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/builtins/viewers.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/builtins/index.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/conversation-draft.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/TabBar.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/split-pane.tsx
		/**
		...
		*  serves both). */
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/desktop-env.ts
		let cached;
		...
		function parseDesktopEnv() {
			...
			const parsedOverlayTop = overlayTopParam === null ? void 0 : Number(overlayTopParam);
			const explicitOverlayTop = parsedOverlayTop !== void 0 && Number.isInteger(parsedOverlayTop) && parsedOverlayTop >= 0 && parsedOverlayTop <= 256 ? parsedOverlayTop : void 0;
			const desktop = mode !== null || typeof window.__DSH_DESKTOP_FILE_PATH__ !== "undefined";
			cached = {
				desktop,
				mode,
				platform,
				win32OverlayTop: desktop && platform === "win32" ? explicitOverlayTop ?? (mode === "advanced" ? 32 : 0) : 0
			};
			return cached;
		}
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/OrphanedTab.tsx
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/RenderBoundary.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/browser-command.ts
		/** Parse one untrusted Host push frame. */
		function parseBrowserCommand(payload) {
			let value;
			try {
			...
			const row = value;
			if (!Number.isInteger(row.id) || row.id < 1) return null;
		...
		* entirely — re-enabling them is the settings page's job. */
		...
		function Sidebar(props) {
			...
			(0, react.useEffect)(() => {
				if (current === void 0 || ctx.betterSidebar === void 0) return;
				let closed = false;
				let socket;
				let retry;
				let failures = 0;
				...
				const connect = () => {
					...
					url.search = new URLSearchParams({ sessionId: current }).toString();
					...
					socket.onmessage = (event) => {
						...
						if (command === null || command.sessionId !== current || ctx.betterSidebar === void 0) return;
						applyBrowserCommand(ctx.betterSidebar, command);
					};
					socket.onclose = () => {
						if (closed) return;
						failures += 1;
						if (failures < FAILURE_LIMIT) retry = window.setTimeout(connect, 2e3);
					};
					socket.onerror = () => {
						socket?.close();
					};
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/link-intercept.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/ime-guard.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/settings-nav-icon.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/prefs.ts
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/plugins-shared.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/plugins-tabs.ts
		/**
		...
		*/
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/plugins-viewers.ts
		/**
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\ChatGPT\RuijieDSH\vendor\dsh-better-sidebar\src\client\SideCardSection.module.css.mjs
		const css$1 = ".vBYrfa_section{flex-direction:column;gap:14px;width:100%;max-width:760px;display:flex}.vBYrfa_intro{color:var(--dsw-alias-label-tertiary);margin:0;padding:0 2px;font-size:13px;line-height:20px}.vBYrfa_group{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:16px;flex-direction:column;flex:none;gap:8px;padding:18px 20px 20px;display:flex}.vBYrfa_groupHeading{color:var(--dsw-alias-label-primary);align-items:baseline;gap:7px;padding:0 2px 6px;font-size:13px;font-weight:600;line-height:20px;display:flex}.vBYrfa_count{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;font-size:12px;font-weight:400;line-height:18px}.vBYrfa_grid{grid-template-columns:repeat(auto-fill,minmax(168px,1fr));gap:10px;display:grid}.vBYrfa_card{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:12px;flex-direction:column;transition:background .12s,border-color .12s;display:flex;position:relative}.vBYrfa_card:not(.vBYrfa_cardOn):hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-label-dimmed)}.vBYrfa_cardOn{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-interactive-bg-active)}.vBYrfa_cardMain{border-radius:inherit;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;flex-direction:column;gap:6px;padding:12px;display:flex}.vBYrfa_cardMain:focus-visible,.vBYrfa_cardGear:focus-visible,.vBYrfa_rowGear:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.vBYrfa_cardTop{align-items:center;gap:8px;min-width:0;display:flex}.vBYrfa_cardIconChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);width:28px;height:28px;color:var(--dsw-alias-label-tertiary);border-radius:8px;flex:none;justify-content:center;align-items:center;display:inline-flex}.vBYrfa_cardOn .vBYrfa_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);background:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 12%, transparent);color:var(--dsw-alias-button-primary-fill)}.vBYrfa_cardTitle{min-width:0;color:var(--dsw-alias-label-secondary);white-space:nowrap;text-overflow:ellipsis;flex:1;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.vBYrfa_cardOn .vBYrfa_cardTitle{color:var(--dsw-alias-label-primary)}.vBYrfa_cardCheck{background:var(--dsw-alias-button-primary-fill);width:16px;height:16px;color:var(--dsw-alias-bg-layer-3);border-radius:50%;flex:none;justify-content:center;align-items:center;display:inline-flex}.vBYrfa_cardDesc{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:11px;line-height:16px;overflow:hidden}.vBYrfa_addCard{border-style:dashed;border-color:var(--dsw-alias-border-l2);text-align:left;align-items:flex-start;padding:12px}.vBYrfa_addCard:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.vBYrfa_addCard:hover .vBYrfa_cardTitle{color:var(--dsw-alias-label-primary)}.vBYrfa_addCard:hover .vBYrfa_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);color:var(--dsw-alias-button-primary-fill)}.vBYrfa_addCard:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.vBYrfa_cardOn .vBYrfa_cardDesc{color:var(--dsw-alias-label-secondary)}.vBYrfa_cardWithGear .vBYrfa_cardDesc{padding-right:30px}.vBYrfa_cardGear{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:50%;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:46px;right:12px}.vBYrfa_cardGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.vBYrfa_rowGear{width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.vBYrfa_rowGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.vBYrfa_row{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:12px 2px;display:flex}.vBYrfa_row:last-child{border-bottom:none}.vBYrfa_rowText{flex-direction:column;gap:4px;min-width:0;display:flex}.vBYrfa_title{color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px}.vBYrfa_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}.vBYrfa_switch{cursor:pointer;flex:none;display:inline-flex;position:relative}.vBYrfa_switchInput{opacity:0;width:1px;height:1px;margin:0;position:absolute}.vBYrfa_switchTrack{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:center;width:36px;height:20px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}.vBYrfa_switchThumb{background:var(--dsw-alias-label-secondary);border-radius:50%;width:14px;height:14px;transition:transform .15s,background .15s;display:block}.vBYrfa_switch:hover .vBYrfa_switchTrack{border-color:var(--dsw-alias-label-dimmed)}.vBYrfa_switchInput:checked+.vBYrfa_switchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}.vBYrfa_switchInput:checked+.vBYrfa_switchTrack .vBYrfa_switchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(16px)}.vBYrfa_switchInput:focus-visible+.vBYrfa_switchTrack{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.vBYrfa_control{flex:none;align-items:center;gap:6px;display:flex}.vBYrfa_percentInput{width:76px}.vBYrfa_typedInput{width:200px}.vBYrfa_typedInputNumber{width:76px}.vBYrfa_selectAnchor{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);max-width:220px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:8px;align-items:center;gap:6px;padding:4px 8px;font-size:13px;line-height:20px;display:flex}.vBYrfa_selectAnchor:hover{border-color:var(--dsw-alias-label-dimmed)}.vBYrfa_selectAnchorIcon{flex:none;display:inline-flex}.vBYrfa_selectAnchorText{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.vBYrfa_selectOption{align-items:center;gap:10px;min-width:200px;display:flex}.vBYrfa_selectOptionIcon{color:var(--dsw-alias-label-secondary);flex:none;display:inline-flex}.vBYrfa_selectOptionText{flex-direction:column;min-width:0;display:flex}.vBYrfa_suffix{color:var(--dsw-alias-label-secondary);font-size:14px;line-height:22px}.vBYrfa_popupDialog.vBYrfa_popupDialog{width:min(460px,100%)}.vBYrfa_popupRows{flex-direction:column;gap:10px;width:100%;display:flex}.vBYrfa_popupRow{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;justify-content:space-between;align-items:center;gap:16px;min-width:0;padding:14px 16px;transition:border-color .16s,background .16s;display:flex}.vBYrfa_popupRow:hover{border-color:var(--dsw-alias-label-dimmed)}.vBYrfa_done{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}.vBYrfa_done:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.vBYrfa_error{color:var(--dsw-alias-state-error-primary);padding:10px 0 2px;font-size:12px;line-height:17px}.vBYrfa_pluginModal.vBYrfa_pluginModal{width:min(560px,100%)}.vBYrfa_pluginList{flex-direction:column;gap:12px;width:100%;display:flex}.vBYrfa_pluginTopicBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:pointer;border-radius:8px;padding:6px 12px;font-size:12px;line-height:18px}.vBYrfa_pluginTopicBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.vBYrfa_pluginTopicBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.vBYrfa_pluginEmpty{color:var(--dsw-alias-label-tertiary);padding:20px 2px;font-size:12px;line-height:18px}.vBYrfa_pluginEntries{flex-direction:column;gap:10px;display:flex}.vBYrfa_pluginEntry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:12px;flex-direction:column;gap:4px;padding:12px;display:flex}.vBYrfa_pluginEntryHead{justify-content:space-between;align-items:center;gap:12px;display:flex}.vBYrfa_pluginEntryActions{flex:none;align-items:center;gap:6px;display:inline-flex}.vBYrfa_pluginJumpBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.vBYrfa_pluginJumpBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.vBYrfa_pluginJumpBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.vBYrfa_pluginName{appearance:none;min-width:0;font:inherit;color:var(--dsw-alias-label-primary);text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;background:0 0;border:0;padding:0;font-size:13px;font-weight:600;line-height:20px;text-decoration:none;overflow:hidden}.vBYrfa_pluginName:hover{color:var(--dsw-alias-button-primary-fill);text-decoration:underline}.vBYrfa_pluginDesc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}.vBYrfa_pluginInstall{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);white-space:nowrap;border-radius:8px;padding:6px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:16px;display:block;overflow-x:auto}.vBYrfa_pluginCopyBtn{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.vBYrfa_pluginCopyBtn:hover{background:var(--dsw-alias-button-primary-hover)}.vBYrfa_pluginCopyBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}@media (prefers-reduced-motion:reduce){.vBYrfa_card,.vBYrfa_switchTrack,.vBYrfa_switchThumb{transition:none}}.vBYrfa_versionBadge{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:999px;align-self:flex-start;align-items:center;gap:8px;padding:4px 12px 4px 14px;font-size:12px;line-height:18px;display:inline-flex}.vBYrfa_versionBadgeName{color:var(--dsw-alias-label-primary);font-weight:600}.vBYrfa_versionBadgeTag{background:var(--dsw-alias-accent-soft,var(--dsw-alias-border-l2));color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;border-radius:999px;padding:1px 8px}.vBYrfa_pluginSearch{box-sizing:border-box;appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);border-radius:8px;padding:6px 10px;font-size:12px;line-height:18px}.vBYrfa_pluginSearch::placeholder{color:var(--dsw-alias-label-tertiary)}.vBYrfa_pluginSearch:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.vBYrfa_pluginEntries{flex-direction:column;gap:10px;max-height:46vh;padding-right:2px;display:flex;overflow-y:auto}.vBYrfa_pluginGroup{flex-direction:column;gap:8px;display:flex}.vBYrfa_pluginGroupHeading{color:var(--dsw-alias-label-secondary);padding:2px 2px 0;font-size:12px;font-weight:600;line-height:18px}";
		...
		var SideCardSection_module_css_default = {
			"addCard": "vBYrfa_addCard",
			"card": "vBYrfa_card",
			"cardCheck": "vBYrfa_cardCheck",
			"cardDesc": "vBYrfa_cardDesc",
			"cardGear": "vBYrfa_cardGear",
			"cardIconChip": "vBYrfa_cardIconChip",
			"cardMain": "vBYrfa_cardMain",
			"cardOn": "vBYrfa_cardOn",
			"cardTitle": "vBYrfa_cardTitle",
			"cardTop": "vBYrfa_cardTop",
			"cardWithGear": "vBYrfa_cardWithGear",
			"control": "vBYrfa_control",
			"count": "vBYrfa_count",
			"desc": "vBYrfa_desc",
			"done": "vBYrfa_done",
			"error": "vBYrfa_error",
			"grid": "vBYrfa_grid",
			"group": "vBYrfa_group",
			"groupHeading": "vBYrfa_groupHeading",
			"intro": "vBYrfa_intro",
			"percentInput": "vBYrfa_percentInput",
			"pluginCopyBtn": "vBYrfa_pluginCopyBtn",
			"pluginDesc": "vBYrfa_pluginDesc",
			"pluginEmpty": "vBYrfa_pluginEmpty",
			"pluginEntries": "vBYrfa_pluginEntries",
			"pluginEntry": "vBYrfa_pluginEntry",
			"pluginEntryActions": "vBYrfa_pluginEntryActions",
			"pluginEntryHead": "vBYrfa_pluginEntryHead",
			"pluginGroup": "vBYrfa_pluginGroup",
			"pluginGroupHeading": "vBYrfa_pluginGroupHeading",
			"pluginInstall": "vBYrfa_pluginInstall",
			"pluginJumpBtn": "vBYrfa_pluginJumpBtn",
			"pluginList": "vBYrfa_pluginList",
			"pluginModal": "vBYrfa_pluginModal",
			"pluginName": "vBYrfa_pluginName",
			"pluginSearch": "vBYrfa_pluginSearch",
			"pluginTopicBtn": "vBYrfa_pluginTopicBtn",
			"popupDialog": "vBYrfa_popupDialog",
			"popupRow": "vBYrfa_popupRow",
			"popupRows": "vBYrfa_popupRows",
			"row": "vBYrfa_row",
			"rowGear": "vBYrfa_rowGear",
			"rowText": "vBYrfa_rowText",
			"section": "vBYrfa_section",
			"selectAnchor": "vBYrfa_selectAnchor",
			"selectAnchorIcon": "vBYrfa_selectAnchorIcon",
			"selectAnchorText": "vBYrfa_selectAnchorText",
			"selectOption": "vBYrfa_selectOption",
			"selectOptionIcon": "vBYrfa_selectOptionIcon",
			"selectOptionText": "vBYrfa_selectOptionText",
			"suffix": "vBYrfa_suffix",
			"switch": "vBYrfa_switch",
			"switchInput": "vBYrfa_switchInput",
			"switchThumb": "vBYrfa_switchThumb",
			"switchTrack": "vBYrfa_switchTrack",
			"title": "vBYrfa_title",
			"typedInput": "vBYrfa_typedInput",
			"typedInputNumber": "vBYrfa_typedInputNumber",
			"versionBadge": "vBYrfa_versionBadge",
			"versionBadgeName": "vBYrfa_versionBadgeName",
			"versionBadgeTag": "vBYrfa_versionBadgeTag"
		};
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/add-plugin-modal.tsx
		/**
		...
		*  with per-entry jump/copy buttons (extracted for direct testing). */
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/SideCardSection.tsx
		/**
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:D:\ChatGPT\RuijieDSH\vendor\dsh-better-sidebar\src\client\layout.css.mjs
		...
		//#endregion
		//#region ../../vendor/dsh-better-sidebar/src/client/index.tsx
		/**
