...
- **`web_search` starts running on modsearch.** dsh already ships a native `web_search` tool over a provider seam, pinned to DeepSeek's keyed search interface. The bundle registers the modsearch engine chain and repoints the seam at it (`searchProvider: modsearch`): the Ruijie desktop distribution prefers configured Exa, Tavily, and agy routes, leaving keyed or keyless Firecrawl as the final fallback while preserving the native citation cards.
