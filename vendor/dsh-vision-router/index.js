...
/** Vectorize an image buffer into an SVG string via local VTracer posterization. */
export function posterizeSvg(bytes, steps = 4, fillStrategy = 'dominant', timeoutMs = 60000) {
  // Vectorization is CPU-bound. Keep it off the main DSH process so other
  // sessions stay responsive and a hard timeout can terminate the work.
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          'vectorization timed out — the image is too large or too complex; crop it to the target region first',
        ),
      )
    }, timeoutMs)
    try {
      const sharpUrl = pathToFileURL(createRequire(import.meta.url).resolve('sharp')).href
      const vectorizerUrl = pathToFileURL(createRequire(import.meta.url).resolve('@neplex/vectorizer')).href
      const source = `
        import('node:worker_threads').then(({ parentPort, workerData }) => {
          Promise.all([import(workerData.sharpUrl), import(workerData.vectorizerUrl)]).then(async ([sharpMod, vectorizer]) => {
            const sharp = sharpMod.default ?? sharpMod
            const decoded = await sharp(Buffer.from(workerData.bytes), { failOn: 'none' })
              .ensureAlpha()
              .raw()
              .toBuffer({ resolveWithObject: true })
            const raw = Buffer.from(decoded.data)
            const levels = Math.max(1, Math.min(16, workerData.steps))
            const step = levels <= 1 ? 255 : 255 / (levels - 1)
            for (let offset = 0; offset < raw.length; offset += 4) {
              const luminance = 0.299 * raw[offset] + 0.587 * raw[offset + 1] + 0.114 * raw[offset + 2]
              const gray = levels <= 1 ? (luminance < 128 ? 0 : 255) : Math.round(luminance / step) * step
              raw[offset] = gray
              raw[offset + 1] = gray
              raw[offset + 2] = gray
            }
      ...
      worker = new Worker(source, {
        eval: true,
        workerData: { sharpUrl, vectorizerUrl, bytes, steps, fillStrategy },
      })
      ...
      worker.once('exit', (code) => {
        if (code !== 0 && !settled) finish(new Error(`vectorizer worker exited with code ${code}`))
      })
    } catch (error) {
      finish(error)
    }
 ...
 * each pixel to the supplied palette, and trace the quantized RGBA buffer
 * with VTracer. Runs in a worker with the same hard timeout and termination
 * semantics as posterizeSvg.
 *
...
export function posterizeSvgColor(data, info, palette, timeoutMs = 60000) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      const source = `
        import('node:worker_threads').then(({ parentPort, workerData }) => {
          import(workerData.vectorizerUrl).then(async (vectorizer) => {
            ...
            for (let p = 0; p < pixels; p++) {
              ...
              raw[o] = paletteRgb[best][0]
              raw[o + 1] = paletteRgb[best][1]
              raw[o + 2] = paletteRgb[best][2]
            }
            const svg = await vectorizer.vectorizeRaw(raw, { width, height }, vectorizer.Preset.Poster)
            parentPort.postMessage({ svg })
          }).catch((error) => {
            parentPort.postMessage({ error: String((error && error.message) || error) })
          })
      ...
      worker.once('error', (error) => finish(error))
      worker.once('exit', (code) => {
        if (code !== 0 && !settled) finish(new Error(`color vectorizer worker exited with code ${code}`))
      })
    } catch (error) {
      finish(error)
    }
...
ctx.logger?.info(
    ...
    deepToolDefs.push({
      name: 'vision_trace',
      description:
        'Vectorize an image (icon/logo) into an SVG via a local VTracer pipeline (no Python). ' +
        'Default: color-preserving vectorization after dominant-color quantization. ' +
      ...
      async execute(args, exec) {
        ...
        // ~1MP (a 1MP bitmap already yields smooth paths), and vectorization cost
        ...
        try {
          ...
          throw new Error(
            `vision_trace: vectorization failed (${error && error.message ? error.message : String(error)})`,
          )
        }
        const target = await saveArtifact(
          exec,
  ...
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => {
      const probe = async () => {
        ...
        // When the free cloud tier is configured first, probe that same first
        // route. `pairs()` preserves the explicit paid row for compatibility,
        // so using it here would misleadingly test the paid fallback instead.
        if (current().freeCloudFirst === true) {
          const freeFirst = httpRouteProviders()[0]
          if (freeFirst !== undefined) return probeModels(freeFirst.baseURL, freeFirst.model)
        }
