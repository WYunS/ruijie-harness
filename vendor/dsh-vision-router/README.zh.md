...
| `vision_trace` | SVG 矢量化（本地 VTracer 分色；图标/logo） | SVG |
...
| `visionTaskTimeoutMs` | `120000` | 每次视觉工具任务的总超时；用于防止单个请求永久卡死，不限制整个长任务 |
| `visionTurnBudgetMs` | `0` | 整轮视觉累计时间上限；`0` 表示不限制，适合付费内部链路和长视频任务 |
| `visionDepth` / `visionDepthMaxCalls` | `custom` / `0` | 每轮成功识图调用上限；`custom + 0` 表示不限制 |
