...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `Ruijie-Harness-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'Ruijie-Harness.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing Ruijie-Harness.exe: ${portablePath}`)
  }
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:Ruijie-Harness.exe`,
  )
  return portablePath
}
