...
/** Permit only the exact inert HTML document generated by this recovery window. */
export function isDesktopStartupRecoveryDocumentNavigation(
  href: string,
  expectedHref: string | undefined,
): boolean {
  return expectedHref !== undefined && href === expectedHref
}
...
export class DesktopStartupRecoveryWindow {
  ...
  async run(): Promise<RecoveryWindowResult> {
    ...
    const navigate = (event: Electron.Event, href: string): void => {
      ...
      if (action === undefined && isDesktopStartupRecoveryDocumentNavigation(href, this.documentUrl)) return
