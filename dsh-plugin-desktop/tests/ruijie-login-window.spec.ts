...
describe('Ruijie SSO startup presentation', () => {
  ...
  it('keeps the login window alive until the Harness window has mounted', () => {
    ...
    const runtimeMount = rendererMount.indexOf('await runtime.mountScheduled()')
    const closed = rendererMount.indexOf('ruijieLoginWindow?.close()')
    expect(authenticated).toBeGreaterThan(0)
    expect(mounted).toBeGreaterThan(authenticated)
    expect(runtimeMount).toBeGreaterThan(0)
    expect(closed).toBeGreaterThan(runtimeMount)
  })
  ...
  it('shows the Harness window after closing the SSO status window', () => {
    ...
    const rendererMount = main.slice(mountDesktopRenderer, mountDesktopRendererEnd)
    const closed = rendererMount.indexOf('ruijieLoginWindow?.close()')
    const shown = rendererMount.indexOf('runtime.show()', closed)
  ...
  it('keeps visible progress between the browser callback and the mounted Harness window', () => {
    ...
    const rendererMount = main.slice(mountDesktopRenderer, mountDesktopRendererEnd)
    const runtimeMount = rendererMount.indexOf('await runtime.mountScheduled()')
    const closed = rendererMount.indexOf('ruijieLoginWindow?.close()', runtimeMount)
