...
describe('published package surface', () => {
  ...
  it('claims plugin install recovery before profile composition and gates health in Electron main', () => {
    ...
    const prepare = main.indexOf('const prepared = prepareDesktopProfile(')
    const mountDesktopRenderer = main.indexOf('const mountDesktopRenderer = async (): Promise<void> => {')
    const mountDesktopRendererEnd = main.indexOf("\n\n  app.on('second-instance'", mountDesktopRenderer)
    const rendererMount = main.slice(mountDesktopRenderer, mountDesktopRendererEnd)
    const monitor = rendererMount.indexOf('runtime.beginRendererBootMonitoring()')
    const mount = rendererMount.indexOf('await runtime.mountScheduled()')
    const normalMount = main.indexOf('\n    await mountDesktopRenderer()\n    const rendererReport = await rendererBoot')
    const awaitRenderer = main.indexOf('const rendererReport = await rendererBoot', normalMount)
    ...
    expect(mountDesktopRenderer).toBeGreaterThanOrEqual(0)
    expect(mountDesktopRendererEnd).toBeGreaterThan(mountDesktopRenderer)
    expect(mount).toBeGreaterThan(monitor)
    expect(normalMount).toBeGreaterThan(prepare)
    expect(awaitRenderer).toBeGreaterThan(normalMount)
