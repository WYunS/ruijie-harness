...
`yarn dist:mac-internal` builds the installable unsigned internal universal DMG on a native macOS host. The same package runs natively on Intel and Apple Silicon Macs. The command refuses non-macOS hosts and runs the complete product gate plus the real Electron sidebar-continuity gate before packaging. It packages without Apple credentials, mounts the DMG, and verifies the property list, executable bit, both `x86_64` and `arm64` slices, the required native runtime files, and `app.asar`. It strips every Electron Builder signing and notarization variable, sets `CSC_IDENTITY_AUTO_DISCOVERY=false`, disables notarization, and never publishes automatically. Because it has no Developer ID signature, another Mac may require a Gatekeeper override for first launch. The separately controlled signed and notarized release remains `yarn dist:mac` and writes to `dsh-plugin-desktop/dist/mac-release/`.
