...
7. 分别切换 Harness 的浅色、深色主题，并覆盖兼容/高级两种窗口模式；右上角最小化、最大化/还原、关闭三个原生按钮必须始终清晰可见、命中区域正常且可点击。标题栏 overlay 必须保持完全透明、自然露出页面背景，只同步原生按钮符号颜色：深色界面用白色符号，浅色界面用深色符号；不得用矩形色块遮盖标题栏，也不能错误跟随与 Harness 不同的 Windows 系统主题。此项不得改为自绘按钮，也不得影响 Windows ConPTY 终端。
