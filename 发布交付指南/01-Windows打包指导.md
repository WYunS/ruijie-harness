...
每次候选包至少用真实模型完成一次回归：让 Python 或终端生成一个中文文件名 PDF，并通过 `dsh_im_return_file` 交付；确认最终回复正文中的 PDF 路径和对话末尾“本次产出”条目都可点击，且两处点击都能在右侧栏打开。若 PDF/Documents 等共享技能输出 Codex 专用的 `:codex-file-citation{...}`，桌面渲染兼容层必须把它转换成 DSH 支持的 Markdown 行内代码文件引用，不能把指令原文暴露给用户。不得只检查磁盘文件存在、工具成功文字或最终回复中出现了文件名；修复前旧会话重新打开后也应能正确转换其已有指令。
