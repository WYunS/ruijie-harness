...
- 右侧栏可以展开、收起；文件标签可以关闭。
- 主程序文件、安装程序、开始菜单、桌面快捷方式、任务栏、窗口和应用切换器均显示紫色 RJ 主应用图标；不得使用旧图标、Electron 默认图标或 macOS 菜单栏专用的黑白 Template 图标。
- 关闭并重启后设置和会话状态符合预期。
...
- `browser_search`：生成 Bing URL 并送到右侧栏，同时读取已加载右栏页面；配置的 `ctx.web` 搜索接缝可并行补充机器可读摘要与来源，但不是右栏读页的前置条件。供应商失败不得把已经成功的导航或本地读页显示成工具错误。
- `web_search`：不打开浏览器，只通过相同 ModSearch 接缝返回真实来源和原生引用卡片；工具名与既有调用方式保持不变。
- `browser_open`：在右侧栏打开指定网址，并在页面加载后返回标题、可见正文和链接；用户随后在右栏内点击到其他页面时，`browser_read_current` 应能读取当前页面供模型总结。
...
2. `browser_search` 的结果页确实加载，而非只返回 `delivered: true`。
   同一次工具结果还必须包含 `evidenceStatus: available` 和非空 `sources`；来源可以来自搜索服务，也可以来自 Electron 隔离 webview 中已加载页面的只读正文提取。模型可据此直接汇总，必要时再调用 `read_page` 交叉核验。
...
故障矩阵必须使用可控夹具覆盖：搜索服务成功时可直接返回来源；搜索服务额度耗尽、限流或不可用而 Chromium 正常时，`browser_search` 必须从已加载的右栏页面返回可读正文和链接，主对话不得显示供应商名称、额度、限流或内部联网错误，失败详情只进入轨迹/日志。右栏页面本身也无法加载时才允许 `evidenceStatus: unavailable`。`web_search` 独立调用仍应存在并保持原有引用卡片与降级能力。
...
- 最终回复正文文件引用、“本次产出”、历史会话文件引用以及 Agent 发起的文件跳转都固定进入右侧栏；即使当前下栏已打开或同一文件此前在下栏显示，也不得改变该规则。只有用户在下栏 Files 文件树中主动点击文件时才继续在下栏预览。
...
IM 文件交付还有一条独立入口：模型可能直接调用 `dsh_im_return_file` 发送已经生成的文件。该工具不能只完成 IM 投递登记；其 `presentCall` 必须同时返回 `card: "generic"`、`kind: "edit"` 和包含原始 `path` 的 `locations`，否则文件虽然真实存在且工具显示 `Registered ... for IM delivery`，对话仍不会把它计入“本次产出”，最终只剩不可点击的灰色文件名。桌面构建会运行 `scripts/patch-dsh-im-runtime.mjs` 注入这项元数据，`tests/dsh-im-runtime-patch.spec.ts` 必须验证最终安装副本中的补丁仍存在。

每次候选包至少用真实模型完成一次回归：让 Python 或终端生成一个中文文件名 PDF，并通过 `dsh_im_return_file` 交付；确认最终回复正文中的 PDF 路径和对话末尾“本次产出”条目都可点击，且两处点击都能在右侧栏打开。正文分别覆盖绝对路径、仅文件名和 `output/pdf/example.pdf` 这类相对子路径；相对子路径必须唯一匹配到该轮绝对产物路径，存在多个候选时不得猜测。若 PDF/Documents 等共享技能输出 Codex 专用的 `:codex-file-citation{...}`，桌面渲染兼容层必须把它转换成 DSH 支持的 Markdown 行内代码文件引用，不能把指令原文暴露给用户。不得只检查磁盘文件存在、工具成功文字或最终回复中出现了文件名；修复前旧会话重新打开后也应能正确转换其已有指令。

`2.1.7` 将附件转换器扩展为 PDF 文字与页面图联合输入以及 ZIP 深度解析。最终 Setup EXE 必须分别上传纯文字 PDF、文字与图片各自包含互补事实的混合 PDF、纯图片扫描 PDF：纯文字文件直接读取文字层，混合文件同时保留文字和页面图并能跨两者推理，扫描件完成 OCR；三类文件均不得导致 Electron Host 退出、白屏或会话中断。源码和安装闭包都必须包含 `tesseract-node-worker.cjs`、OCR 语言数据及 PDF 转换脚本，并运行 `verify:attachment-ocr-electron`，确认 Tesseract worker 在 Electron 主进程下不会因 Node 环境误判而崩溃，单页 OCR 失败只形成可见的附件错误。

ZIP 不能只列目录。候选包必须用真实 ZIP 验证安全解压后递归读取其中的文本、代码、PDF、Word、Excel、PowerPoint 和图片，生成完整目录树、内容摘录及 PDF 页面预览，并让真实模型基于包内事实回答问题。至少覆盖中文与 GBK 文件名、GBK 文本、嵌套目录、空目录和同名文件；包含 `SKILL.md` 时要标出技能入口并保留其引用文件树，使模型能把技能内容作为用户提供的学习材料使用，但包内文字不能自动提升为用户指令。路径穿越、绝对路径、符号链接、异常压缩比、超大展开体积、文件数超限、损坏或加密归档必须安全拒绝，不得在目标目录外写入、静默漏读或拖垮 Host。缓存命中前要核对声明的全部转换产物仍存在，不能复用残缺缓存。
...
针对已确认的 GPTAuth 异常落点，桌面端只允许一次自动恢复：授权地址同源主页，或授权链路已访问 `sid.ruijie.com.cn` 后误落的 HTTPS `/user`、`/dashboard` 及其子路由，可以重新加载最初的完整 `/oauth/authorize` URL 一次；同一尝试的第二次主页落点不得再次恢复。没有访问企业 SSO 的跨域 `/user`、普通 HTTP 页面、正常 `localhost` callback、普通 sign-in 和其他路径都不得触发。必须用真实重定向序列以及至少 20 次 sign-in、50 次 dashboard 噪声事件验证最终只发生一次恢复，不允许轮询、定时重试或形成频闪循环。

正常回调到 `http://localhost:<port>/auth/callback` 后，同一个授权窗口切换为黑白居中的锐捷 Harness 状态页：先显示“正在验证账号”，账号网络超过 8 秒显示中性的网络/代理提示；账号验证完成后才显示“认证已完成”，工作台加载超过 8 秒再显示组件初始化提示。状态页使用 72 px 黑白 RJ 标记、居中品牌文字和黑色细线进度，不恢复旧版蓝紫卡片、左上品牌块或额外弹窗。令牌交换、刷新和模型接口验证必须有 30 秒超时，不能无限停留或过早宣称认证完成。

在**专用测试账户**分别验证：全新用户首次 SSO、已登录 SSO 用户、保存凭据的老用户、显式退出后重登、授权前关闭、SSO 输入账号后关闭、callback 后验证阶段关闭、脏默认浏览器 Cookie、重复 sign-in/dashboard 导航以及断网/代理异常。关闭授权窗口应只结束当前启动，不要求关闭第二个窗口，也不得立即重弹；取消授权不得误删其他账号资料。记录每个阶段可见 URL、恢复次数、状态文字、网络/代理/PAC、耗时和日志。频闪、同一路径持续往返、一次启动出现两个授权窗口、成功登录后停在主页、callback 已收到但工作台不启动，均为发布阻断。

`2.1.1` 首次交付本方案，Windows 候选包必须把本节全部登录项列为 `release-blocking`。源码单测、本地开发版肉眼通过或另一平台通过，都不能替代最终 Setup EXE 安装版的操作证据；任一项失败、未执行或证据不足时，不得把 `2.1.1` 标记为 Windows 可发布版。
...
1. 全新 profile 不进入插件市场、不执行安装即可在“通用设置”顶部看到“界面外观（颜色、壁纸与透明度）”。该行收起时也必须始终显示“一键恢复默认”；先修改多项颜色、背景、透明度和侧栏效果，再点击恢复，必须一次清除全部外观自定义并回到原版界面，同时不影响登录、账号、会话、工作区或 IM 配置。左侧栏底部从上到下应为账号/额度、“IM机器人”、设置；“IM机器人”与原生“设置”的文字同色、行高与左边界一致，机器人图标和齿轮图标视觉尺寸一致，点击必须直达“插件 → IM机器人”，不得打开插件市场。收起后账号显示首字并带与主图标一致的紫色底，三个入口肉眼对齐且均可正常点击；展开状态的既有像素位置不得被收起态修正带偏。首次打开和点击“恢复默认”后，用户消息气泡必须回到系统当前浅色/深色模式对应的原版颜色；用户主动选择“主色”后，按钮、链接、选中态和用户气泡应立即一致跟随，完全退出重启后保持。
...
7. 分别切换 Harness 的浅色、深色主题，并逐一抽测 Default、Midnight、Ocean、Forest、Rose、Monochrome 预设以及兼容/高级两种窗口模式；右上角最小化、最大化/还原、关闭三个原生按钮必须始终清晰可见、命中区域正常且可点击。标题栏 overlay 必须保持完全透明、自然露出当前页面背景，只同步原生按钮符号颜色：深色界面用白色符号，浅色界面用深色符号；切换主题应立即生效且无需重启，不得出现白块、黑块或固定色矩形，也不能错误跟随与 Harness 不同的 Windows 系统主题。此项不得改为自绘按钮，也不得影响 Windows ConPTY 终端。
