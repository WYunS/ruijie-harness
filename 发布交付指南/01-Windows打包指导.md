...
每次候选包至少用真实模型完成一次回归：让 Python 或终端生成一个中文文件名 PDF，并通过 `dsh_im_return_file` 交付；确认工具成功后，对话末尾“本次产出”包含该 PDF，点击能在右侧栏打开。不得只检查磁盘文件存在、工具成功文字或最终回复中的文件名。旧会话在修复前已经固化的调用元数据可以不被追溯补写，验收必须在升级后的新回合重新生成并交付。
