...
After confirmation, the app downloads the installer into its private update directory and remembers the location without asking the user to choose a save path. macOS opens the DMG for the user to replace the application in Applications; Windows prepares the NSIS installer and then asks whether to quit and run the in-place upgrade, without requiring an uninstall. After the upgrade completes and the app starts again, it asks whether to delete the installer to free disk space or keep it. Download or installer failures do not damage the current version, and the tray operation can be retried.
