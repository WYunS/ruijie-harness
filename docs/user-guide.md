...
打包后的 Windows 应用会在后台检查 `https://gptauth.ruijie.com.cn/harness/api/desktop/version/windows`，macOS 应用检查对应的 `/version/mac`。两个平台可独立发布不同版本；后台检查不阻塞启动，网络错误、非 200、非法版本或服务端版本不新时保持静默。

托盘中的 **Check for Updates…** 是手动检查：即使已经是当前版本，也会显示结果；检查失败会提示稍后重试。只有服务端版本严格高于本地版本时，应用才会询问是否下载。用户取消不会访问计数下载入口。

确认下载后，应用会把安装包下载到自己的私有更新目录并记录位置，不再要求用户选择保存路径。macOS 会自动打开 DMG，由用户把应用替换到 Applications；Windows 会准备 NSIS 安装器，再询问是否退出并启动覆盖安装，无需先卸载旧版。升级完成并重新启动后，应用会询问是否删除安装包以释放磁盘空间，也可以选择保留。下载和安装失败不会破坏当前版本，托盘仍可重试。
